//22.Should allow adding a child item to parent item in structure manager when a property “source” value  is same on both.
#include <iostream>
...
#define PLM_error (EMH_USER_error_base +16)
	...
	extern DLLAPI int ADD_pre_condition(METHOD_message_t msg, va_list argv)
	{
		...
		AOM_ask_value_string(parent, "bl_item_item_id", &item_id);
		ifail = ITEM_find_item(item_id, &tItem);
		ifail = ITEM_ask_latest_rev(tItem, &tRev);
		AOM_ask_value_int(tRev, "source", &iPart_source);
		cout << "cPart_source : " << iPart_source << endl;
		AOM_ask_value_int(item_revision, "source", &iValue);
		cout << "cValue : " << iValue << "\n";

		if (iPart_source == iValue)
		{
