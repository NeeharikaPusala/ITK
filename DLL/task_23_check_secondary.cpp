	...
	extern DLLAPI int PLM_add_post_action(METHOD_message_t *msg, va_list argv)
	{
		...
		if (iGRM == 1)
		{
			...
			if (ifail == ITK_ok && tClass_id != NULLTAG)
			{
				...
				{
					snprintf(dataset_name, sizeof(dataset_name), "%s-%s-%d", cItem_id, cRev_id, iGRM);
		...
		else
		{
			if (iGRM >= 1)
			{
				for (int i = 0; i < iGRM; i++)
				{
					
					ifail = POM_class_of_instance(tSec_objs[i], &tClass_id);
					ifail = POM_name_of_class(tClass_id, &cName_class);
					AOM_ask_value_string(tSec_objs[i], "object_name", &cname_val);
					if (tc_strcmp(cName_class, "Dataset") == 0)
					{
						snprintf(dataset_name, sizeof(dataset_name), "%s-%s-%d", cItem_id, cRev_id, iGRM);
						if (tc_strcmp(cname_val, dataset_name) == 0)
						{
							iGRM++;
							snprintf(dataset_name, sizeof(dataset_name), "%s-%s-%d", cItem_id, cRev_id, iGRM);
							AOM_refresh(secondary_object, true);
						...
						size_t lastHyphen = string(cname_val).rfind('-');
						if (lastHyphen == string::npos)
						{

							tEmpty = tSec_objs[i];
							
							snprintf(dataset_name, sizeof(dataset_name), "%s-%s-%d", cItem_id, cRev_id, iGRM);
							AOM_refresh(tEmpty, true);
							ifail = AOM_set_value_string(tEmpty, "object_name", dataset_name);
							AOM_save_without_extensions(tEmpty);
							AOM_refresh(tEmpty, false);
							continue;
